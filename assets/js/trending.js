...
const TrendingPage = {
        ...
        if (tabs) {
            tabs.querySelectorAll('.trending-tab').forEach(btn => {
                btn.addEventListener('click', () => {
                    ...
                    this.window = btn.dataset.window || 'week';
                    this.reset();
                });
            });
        }
    ...
    async load() {
        ...
        try {
            const data = await Api.get(`/trending.php?window=${this.window}&page=${this.page}`);
...
if (document.getElementById('trending-page')) TrendingPage.init();
